  ...
  //////////////////////////////////////////////////////////////
    ...
    //Сколько еще в буфере ?!
                ...
                begin
                    ...
                    begin
                        BytesInStack := $FFFE;
                    end; //В прочитаном буффере - не более чем то что можем скушать за раз.
                    if BytesInStack > 0 then
                    begin//Дочитываем
...
begin
  ...
  // получаем запрос на подключение
    SetLength(buf, 4);
    if (not GetSocketData(sock, buf[1], 4)) or (buf[1] <> #5) // проверяем версию протокола
      ...
      //Метод авторизации
        case Buf[2] of
          ...
          //и ждем ответа
            ...
            begin
                beep;
            end; //Дисконнект. неподдерживаемый метод авторизации.
        end;
        ...
        begin
            ...
            begin
                ...
                case buf[2] of
                    ...
                    begin
                        result := 8;
                    end;//'ошибка SOCKS-сервера';
                    #$02 :
                    begin
                        result := 9;
                    end;//'соединение запрещено набором правил';
                    #$03 :
                    begin
                        result := 10;
                    end;//'сеть недоступна';
                    ...
                    begin
                        result := 14;
                    end;//'команда не поддерживается';
                    #$08 :
                    begin
                        result := 15;
                    end;//'тип адреса не поддерживается';
                end;
                exit;
            end;
        end;
