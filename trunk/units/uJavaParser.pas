...
//    procedure ClearStatement;
    ...
    public
        ...
        procedure Translate(PacketName, Packet : string; size : word; var offset : integer; var typ, name_, value, hexvalue : string);
        function ParseString() : boolean;
        function FindToken(tok : string) : integer;
        function GetValueFromPktStr(const typ, name_, PktStr : string; var PosInPkt : integer; var hexvalue : string) : string;
    { Current position }
...
begin
    ...
    begin
        s := name + '=0';
    end;  //попробуем вместо пустой переменной ноль ставить
    if i > -1 then
    begin
        Fvars[i] := s;
    end    //если уже есть, то обновляем
    else
    begin
        Fvars.Append(s);
    end;  //дописываем новую переменную
...
begin
    ...
    begin
        FPosition := index;
    end  //устанавливаем текущ. строку
    else
    begin
        FPosition := Count - 1;
    end;  //тогда на последнюю строку
end;
//******************************************************************************
...
//******************************************************************************
function TJavaParser.GetValueFromPktStr(const typ, name_, PktStr : string; var PosInPkt : integer; var hexvalue : string) : string;
var
    value : string;
    d : integer;
    pch : widestring;
...
//******************************************************************************
procedure TJavaParser.Translate(PacketName, Packet : string; size : word; var offset : integer; var typ, name_, value, hexvalue : string);
var
    i : integer;
    s : string;
begin
