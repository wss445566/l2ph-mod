...
function WSPSend(s : TSocket; lpBuffers : LPWSABUF; dwBufferCount : DWORD; var lpNumberOfBytesSent : DWORD; dwFlags : DWORD; lpOverlapped : LPWSAOVERLAPPED; lpCompletionRoutine : LPWSAOVERLAPPED_COMPLETION_ROUTINE; lpThreadId : LPWSATHREADID; var lpErrno : integer) : integer; stdcall;
var
  Client : TClient;
begin
  Client := GetConnectionData(s);
...
function WSPioctrl(s : TSocket; dwIoControlCode : DWORD; lpvInBuffer : LPVOID; cbInBuffer : DWORD; lpvOutBuffer : LPVOID; cbOutBuffer : DWORD; var lpcbBytesReturned : DWORD; lpOverlapped : LPWSAOVERLAPPED; lpCompletionRoutine : LPWSAOVERLAPPED_COMPLETION_ROUTINE; lpThreadId : LPWSATHREADID; var lpErrno : integer) : integer; stdcall;
begin
  ...
  begin
    debug('WSPioctrl result (' + inttostr(result) + ') s (' + inttostr(s) + ') dwIoControlCode (' + inttostr(dwIoControlCode) +
      ') cbInBuffer (' + inttostr(cbInBuffer) + ') cbOutBuffer (' + inttostr(cbOutBuffer) + ') lpcbBytesReturned (' + inttostr(lpcbBytesReturned) + ') lpThreadId (' + inttostr(lpThreadId.ThreadHandle) + ')' + ') lpErrno (' + inttostr(lpErrno) + ')');
  end;
end;
...
begin
  ...
  begin
    debug('WSPAsyncSelect s (' + inttostr(s) + ') hWnd (' + inttostr(hWnd) +
      ') wMsg (' + inttostr(wMsg) + ') lEvent (' + inttostr(lEvent) + ') lpErrno (' + inttostr(lpErrno) + ')');
  end;
